...
from models import Ride, User, Rating
...
def find_matches_for_request(request, max_results=5):
    candidates = Ride.query.filter(Ride.status == 'planned', Ride.seats_available >= request.seats_needed).all()
    scored = []
    for ride in candidates:
        ...
        if time_diff_minutes > max_flex:
            continue
        max_dist = 0.5 if ride.zero_stress_mode else 1.5
        if ride.start_lat and request.start_lat:
            dist = geodesic((request.start_lat, request.start_lon), (ride.start_lat, ride.start_lon)).km
            if dist > max_dist:
                continue
        else:
            dist = 1.0
        ...
        if ratings:
            avg_rating = sum((r.cleanliness or r.politeness or 3) + (r.driving_style or r.punctuality_passenger or 3) +
                (r.comfort or r.tidiness or 3) + (r.communication or 3) + (r.punctuality or 3) for r in ratings) / (5 * len(ratings))
            rating_score = avg_rating / 5.0
        else:
            rating_score = 0.5
        total_score = 0.35 * time_score + 0.2 * distance_score + 0.3 * compat_score + 0.15 * rating_score
        scored.append({'ride': ride, 'driver': driver, 'score': round(total_score, 2),
            'compatibility': round(compat_score * 100), 'time_score': round(time_score, 2),
            'distance_score': round(distance_score, 2), 'price': ride.price_per_seat,
