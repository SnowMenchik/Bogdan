...
def create_route_page():
    if request.method == 'POST':
        ...
        route = create_route(current_user.id, start_point, end_point, departure_time,
            ','.join(days_of_week) if days_of_week else '', available_seats,
            zero_stress_mode, music_pref, talk_pref, ac_pref, smoking)
        if ride_date:
            dt = None
            try:
...
def create_request_page():
    if request.method == 'POST':
        ...
        if dt:
            req = create_request(current_user.id, start_point, end_point, dt,
...
def ride_detail(ride_id):
    ...
    if not ride:
        ...
        return redirect(url_for('rides.ride_list'))
    return render_template('ride_detail.html', ride=ride, matches=ride.matches)
...
def cancel_ride(ride_id):
    ...
    for m in ride.matches:
        if m.status == 'accepted':
            m.status = 'rejected'
            create_notification(m.request.passenger_id, f'Поездка {ride.start_point} -> {ride.end_point} отменена.')
    db.session.commit()
...
def complete_ride(ride_id):
    ...
    for m in ride.matches:
        if m.status == 'accepted':
            m.status = 'completed'
            create_notification(m.request.passenger_id, f'Поездка завершена. Оцените водителя!')
    create_notification(current_user.id, 'Поездка завершена. Оцените пассажиров!')
    db.session.commit()
    flash('Поездка завершена!', 'success')
    return redirect(url_for('rides.ride_list'))

@rides_bp.route('/search_address')
@login_required
def search_address():
    from integrations.twogis import search_address as twogis_search
    query = request.args.get('q', '')
    results = twogis_search(query)
    return {'results': results}

@rides_bp.route('/cancel_request/<int:request_id>', methods=['POST'])
@login_required
def cancel_request(request_id):
    ...
    if not req or req.passenger_id != current_user.id:
        ...
        return redirect(url_for('rides.ride_list'))
    if req.status != 'open':
        flash('Можно отменить только открытые запросы.', 'warning')
        return redirect(url_for('rides.ride_list'))

    req.status = 'cancelled'
    # Cancel related matches
    for m in req.matches:
        if m.status in ('proposed', 'accepted'):
            m.status = 'cancelled'
            # Notify driver
            ride = db.session.get(Ride, m.ride_id)
            if ride:
                create_notification(ride.driver_id,
                    f'Пассажир {current_user.full_name} отменил запрос на поездку {ride.start_point} -> {ride.end_point}.')
    db.session.commit()
    flash('Запрос отменён.', 'info')
    return redirect(url_for('rides.ride_list'))
...
def parse_text():
    ...
    if result:
        ...
        flash('Не удалось распознать запрос.', 'error')
        return redirect(url_for('rides.create_request_page'))
